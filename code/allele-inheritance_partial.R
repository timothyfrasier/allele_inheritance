...
for (i in 1:nLoci) {
  ...
  temp <- fread(file, header = FALSE, sep = ",", select = c(i * 2, (i * 2) + 1))
...
for (i in 1:nTriads) {
  ...
  for (j in 1:nLoci) {
    ...
    #---------------------------------------------------------#
    parents1 <- fread(pfile, header = FALSE, sep = ",", select = c(j * 2, (j * 2) + 1))
    parents <- parents1[((i * 2) - 1):(i * 2), ]

    offspring1 <- fread(ofile, header = FALSE, sep = ",", select = c(j * 2, (j * 2) + 1))
    offspring <- offspring1[i, ]
...
file.create("simOff.csv")

for (i in 1:nTriads) {
  
  # Vector to hold results
  offspring <- rep(NA, times = ((2 * nLoci) + 1))
  ...
  for (j in 1:nLoci) {
    ...
    if (sum(parents == 0) > 0) {
      ...
      offspring[(j * 2)] <- sample(as.numeric(parents[1, ]), size = 1, replace = FALSE)
      offspring[(j * 2) + 1] <- sample(as.numeric(parents[2, ]), size = 1, replace = FALSE)
    }
  }
  write.table(matrix(offspring, nrow = 1), file = "simOff.csv", append = TRUE, sep = ",", 
